# xterax-shell-integration (PowerShell)
...
if ($global:__XTERAX_HOOKS_LOADED) { return }
$global:__XTERAX_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__xterax_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__xterax_urlencode {
    param([string]$s)
...
function global:__xterax_install_readline {
    if ($global:__xterax_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__xterax_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__xterax_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __xterax_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__xterax_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__xterax_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($env:XTERAX_BLOCKS) {
        $global:LASTEXITCODE = $lec
        $gap = if ($global:__xterax_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__xterax_user_prompt) {
        try { & __xterax_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
