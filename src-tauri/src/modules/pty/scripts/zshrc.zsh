# xterax-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_xterax_ret`.

{
  _xterax_user_zdotdir="${XTERAX_USER_ZDOTDIR:-$HOME}"
  [ -f "$_xterax_user_zdotdir/.zshrc" ] && source "$_xterax_user_zdotdir/.zshrc"
...
if [[ -z "$__XTERAX_HOOKS_LOADED" ]]; then
  __XTERAX_HOOKS_LOADED=1
  ...
  _xterax_precmd() {
    local _xterax_ret=$?
    printf '\e]133;D;%s\e\\' "$_xterax_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_xterax_urlencode "$PWD")"
  ...
  _xterax_preexec() {
    ...
    [[ -n "$XTERAX_BLOCKS" ]] && _xterax_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _xterax_precmd
    add-zsh-hook preexec _xterax_preexec
  fi
