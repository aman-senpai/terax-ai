...
pub fn wsl_path_to_unc(distro: &str, path: &str) -> PathBuf {
    ...
    if !is_safe_distro_name(distro) {
        return PathBuf::from(r"\\wsl.localhost\__xterax_invalid_distro__");
    }
    let normalized = path.replace('\\', "/");
...
mod tests {
    ...
    fn wsl_path_to_unc_blocks_traversal_distro() {
        ...
        assert!(s.contains("__xterax_invalid_distro__"), "got: {s}");
...
mod auth_tests {
    ...
    fn tempdir(label: &str) -> PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        p.push(format!("xterax-auth-{label}-{nanos}-{}", std::process::id()));
    ...
    fn authorize_user_spawn_cwd_rejects_missing_path() {
        let mut missing = env::temp_dir();
        missing.push(format!("xterax-user-missing-{}", std::process::id()));
        let reg = WorkspaceRegistry::default();
        let s = missing.to_string_lossy().into_owned();
    ...
    fn resolve_launch_cwd_ignores_nonexistent_cli_dir() {
        ...
        let resolved = resolve_launch_cwd(Some("/no/such/xterax/dir"), Some(env.clone()));
...
mod appimage_tests {
    ...
    fn strips_appdir_from_path_lists_and_unsets_when_empty() {
        let appdir = Path::new("/tmp/.mount_Xterax_X");
        let env = reader(&[
            ("LD_LIBRARY_PATH", "/tmp/.mount_Xterax_X/usr/lib:/usr/lib"),
            ("PATH", "/tmp/.mount_Xterax_X/usr/bin:/usr/bin:/bin"),
            ("GST_PLUGIN_SYSTEM_PATH", "/tmp/.mount_Xterax_X/usr/lib/gstreamer-1.0"),
            ("APPDIR", "/tmp/.mount_Xterax_X"),
        ]);
    ...
    fn unsets_value_vars_only_when_pointing_into_appdir() {
        let appdir = Path::new("/tmp/.mount_Xterax_X");
        let into = reader(&[("LD_PRELOAD", "/tmp/.mount_Xterax_X/usr/lib/x.so")]);
