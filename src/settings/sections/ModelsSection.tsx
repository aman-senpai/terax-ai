...
export function ModelsSection() {
  ...
  return (
    <div className="flex flex-col gap-7">
      <SectionHeader
        title="Models"
        description="Connect the providers you use. Keys live in your OS keychain and are used only by Xterax."
      />
...
function DefaultsBlock({
  ...
  const defaultThinkingLevel = usePreferencesStore(
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2.5 rounded-lg border border-border/60 bg-card/60 px-3 py-2.5">
        ...
        <ProfileModelRow keys={keys} configuredIds={configuredIds} />
      </div>
    </div>
  );
}
...
function DefaultModelPicker({
  ...
  const currentThinking =
    thinkingLevels.find((l) => l.value === defaultThinkingLevel) ??
    thinkingLevels[0];
              ...
              >
                {thinkingLevels.map((l) => (
                  <DropdownMenuItem
                    key={l.value}
                    onSelect={() =>
                      void setAutocompleteThinkingLevel(
              ...
              >
                {thinkingLevels.map((l) => (
                  <DropdownMenuItem
                    key={l.value}
                    onSelect={() =>
                      void setSubagentThinkingLevel(l.value as ThinkingLevel)
                    }
                    className={cn(
                      "text-[11.5px]",
                      l.value === thinkingLevel && "bg-accent/50",
...
function ProfileModelRow({
  ...
  const provider = usePreferencesStore((s) => s.profileProvider);
  const modelId = usePreferencesStore((s) => s.profileModelId);
  ...
  const hasKey = provider
    ? providerNeedsKey(provider as ProviderId)
      ...
      : true
    : true;
  const canThink = provider
    ? supportsThinkingLevel(provider as ProviderId)
    : false;
  const thinkingLevels = provider
    ? getThinkingLevels(provider as ProviderId)
    : [];
  const currentThinking =
    thinkingLevels.find((l) => l.value === thinkingLevel) ?? thinkingLevels[0];
  ...
  return (
    <>
      <FieldRow label="Engineering profile">
        <div className="flex flex-1 items-center gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
            ...
            <DropdownMenuContent
              ...
              className="max-h-72 min-w-70 overflow-y-auto"
            >
              {PROVIDERS.some((p) => grouped.get(p.id)?.length) ? (
                <DropdownMenuLabel className="px-2 text-[10px] tracking-wide text-muted-foreground uppercase">
                  Models
                </DropdownMenuLabel>
              ) : null}
              ...
              >
                {thinkingLevels.map((l) => (
                  <DropdownMenuItem
                    key={l.value}
                    onSelect={() =>
                      void setProfileThinkingLevel(l.value as ThinkingLevel)
                    }
                    className={cn(
                      "text-[11.5px]",
                      l.value === thinkingLevel && "bg-accent/50",
      ...
      {!hasKey && provider ? (
        <p className="pl-19 text-[10.5px] text-muted-foreground">
          {getProvider(provider as ProviderId).label} isn't connected — add it
          below.
        </p>
      ) : null}
    </>
