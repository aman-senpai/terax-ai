...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S xterax-bin";
    case "debian":
      return `sudo apt install ./Xterax_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Xterax-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Xterax v${manual.version} is available`
                  : `Xterax v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Xterax to finish installing."
              : downloading
                ? progress !== null
