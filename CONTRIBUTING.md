# Contributing to Xterax

Xterax is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- Xterax has one active maintainer ([@crynta](https://github.com/crynta)).
...
Xterax positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
The most common way a PR breaks Xterax is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
- Xterax is not trying to be a full IDE replacement (VS Code, Cursor, Zed).
